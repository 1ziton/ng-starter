...
import { AfterViewInit, ChangeDetectorRef, Component, ContentChild, EventEmitter, forwardRef, Input, OnInit, Output, Renderer2, TemplateRef } from '@angular/core';
...
@Component({
  ...
  providers: [{
    ...
    useExisting: forwardRef(() => AreaMultipleSelectCom),
    multi: true
  }]
})
export class AreaMultipleSelectCom implements OnInit, AfterViewInit, ControlValueAccessor {
  ...
  echoSelet() {
    ...
    this.checkedCityArr = [..._.map(this.selection, 'cityId'),
    ..._.map(_.reject(this.selection, ['cityId', 0]), 'selectId')];
    this.checkedDistrictArr = [..._.map(this.selection, 'districtId'),
  ...
  checkedPro(pro, label, $event) {
    ...
    if (label.checked) {
      pro.click = true;
      if (pro.children.length) { pro.children = [] }
  ...
  checkedCity(city, label, $event) {
    ...
    if (label.checked) {
      city.click = true;
      //如果有区 就情空
      if (city.children.length) { city.children = [] }
      ...
      this.selection = this.selection.filter(item => {
        if (item.cityId === city.id || item.selectId === city.id) { //item.selectId === city.id 全市
          this.checkedDistrictArr.splice(this.checkedDistrictArr.indexOf(item.districtId), 1);
          return false;
        }
        return true;
  ...
  setChecked(type, data) {
    ...
    let arr: any[] = [];
    type === 'cityData' ? arr = checkedCityArr : type === 'districtData' ? arr = checkedDistrictArr : arr = checkedProArr;
    data.forEach((v) => {
      v.click = false;
  ...
  handleResult(type?) {
    ...
    this.selection = this.selection.filter(item => {
      if (_.uniq(chenkArr).includes(item.selectId)) {
        seletedId = [
          _.get(item, 'provinceId'),
          _.get(item, 'cityId'),
          _.get(item, 'districtId'),
          ...seletedId
        ]
        return true
      }
